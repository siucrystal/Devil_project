      ...
      $.ajax({
           ...
           success: function(data) {
            ...
            for(let i=0;i<data.list.length;i++){
                ...
                const sodium = list[i].sodium
                const date = list[i].date
                put += '<div class="menu_body myfood_flexStyle"><span>' + name + '</span><span><input type="number" id="amount' + idx + '" value="' + amount + '"></span><span>' + energy + '</span><span>' + carb + '</span><span>' + sugar + '</span><span>' + protein + '</span><span>' + fat + '</span><span>' + sodium + '</span>'
